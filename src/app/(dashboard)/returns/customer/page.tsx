  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl md:text-3xl font-extrabold text-slate-650">↩️ مرتجعات العملاء</h1>
          <p className="text-sm text-gray-500">
            {data?.total ?? "..."} مرتجع • إجمالي: {formatEGP(totalAmount)} جنيه
          </p>
        </div>
        <Link href="/returns/customer/new" className="btn-primary">+ مرتجع جديد</Link>
      </div>
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="space-y-2 md:hidden">
            ...
            {data?.items.length === 0 && (
              <div className="card text-center py-12 text-gray-400">لا توجد مرتجعات</div>
            )}
          </div>
          ...
          <div className="card overflow-x-auto p-0 hidden md:block">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {data?.items.length === 0 && (
                  <tr><td colSpan={7} className="p-12 text-center text-gray-400">لا توجد مرتجعات عملاء</td></tr>
                )}
              </tbody>
            </table>
          </div>
...
============================================================ */
function CustomerReturnDetailsModal({
      ...
      <div onClick={e => e.stopPropagation()} className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-white border-b p-4 flex items-center justify-between gap-2 z-10">
          <div>
            <h2 className="text-lg md:text-xl font-bold">
              ↩️ مرتجع عميل #{ret.return_number}
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-sm">
            <ReturnInfo label="العميل" value={ret.customer?.name} />
          ...
          <div className="border-t pt-3">
            <div className="flex justify-between text-lg font-extrabold text-orange-700">
              <span>إجمالي المرتجع:</span>
              <span className="font-mono">{formatEGP(Number(ret.total_amount))} ج</span>
            </div>
            <p className="text-xs text-gray-500 mt-1">
              ✅ تم إضافة الكميات للمخزون وخصم المبلغ من رصيد العميل عند إنشاء هذا المرتجع.
            </p>
          </div>
