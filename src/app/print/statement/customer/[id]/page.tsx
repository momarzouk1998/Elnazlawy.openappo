    ...
    <div className="min-h-screen bg-[#f0f2f5] p-4 print:p-0 print:bg-white">
      ...
      <div className="print-page max-w-[800px] mx-auto bg-white shadow-2xl rounded-xl overflow-hidden">
        ...
        <div className="divide-y divide-gray-100">
          ...
          {entries.map((e, i) => (
            <div key={i} className={`${e.type === 'invoice' ? 'bg-white' : e.type === 'return' ? 'bg-orange-50' : e.type === 'payment' ? 'bg-green-50' : 'bg-gray-50'}`}>
              ...
              {/* ── Items sub-table (فواتير ومرتجعات) ── */}
                ...
                <table className="w-full text-[0.78em] border-t border-dashed border-gray-200 mb-1">
                  <thead>
                    <tr className="bg-gray-100 text-gray-500">
                      ...
                      <th className="px-2 py-1 text-left font-medium w-20">السعر</th>
                  ...
                  <tbody>
                    {e.items.map((it, j) => (
                      <tr key={j} className="border-t border-gray-100">
                        ...
                        <td className="px-2 py-1 text-left font-mono">{formatEGP(it.unit_price)}</td>
        ...
        <div className="grid grid-cols-[1fr_90px_90px_90px] gap-x-3 px-3 py-1.5 bg-nazlawy-500 text-white text-[0.78em] font-bold border-t-2 border-nazlawy-600 mt-1">
          ...
          <div className="text-left">مدين (عليه)</div>
          <div className="text-left">دائن (له)</div>
        ...
        <div className="mx-5 my-4 grid grid-cols-3 gap-3 text-sm text-center">
          <div className="border rounded-lg p-3">
            <div className="text-gray-500 text-xs mb-1">إجمالي المبيعات</div>
            <div className="font-extrabold font-mono text-blue-700">
              {formatEGP(invoices.reduce((s, inv) => s + Number(inv.total), 0))} ج
            </div>
          </div>
          <div className="border rounded-lg p-3">
          ...
          <div className="border rounded-lg p-3">
            <div className="text-gray-500 text-xs mb-1">إجمالي التحصيلات</div>
          ...
          <div className="border rounded-lg p-3 col-span-2">
            <div className="text-gray-500 text-xs mb-1">صافي المبيعات (بعد المرتجعات)</div>
            <div className="font-extrabold font-mono text-nazlawy-700">
              {formatEGP(
                invoices.reduce((s, inv) => s + Number(inv.total), 0) -
