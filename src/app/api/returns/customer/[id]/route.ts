...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const ret = await prisma.customer_return_invoices.findUnique({
    where: { id },
    include: {
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    // إلغاء المرتجع = عكس التأثير (خصم المخزون + إرجاع رصيد العميل)
    await prisma.$transaction(async (tx) => {
      // خصم المخزون (عكس الزيادة عند الإنشاء)
      for (const item of ret.items) {
        ...
        if (available < Number(item.quantity)) {
          throw new Error(`لا يوجد مخزون كافٍ لإلغاء هذا المرتجع. الصنف "${item.product_name}" متاح منه ${available} فقط.`);
        }
      ...
      await tx.customer_return_invoices.update({
        where: { id },
