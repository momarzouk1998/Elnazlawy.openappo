...
| **Host** | اسم الـ container (يتم تحديده تلقائياً في GitHub Actions) |
| **Port** | `5432` |
| **Database Name** | `mazaya_factory` |
| **User** | `mazaya` |
| **Password** | `mazaya_pass_123` |

### Connection String (للإنتاج)
```
postgres://mazaya:mazaya_pass_123@<container-host>:5432/mazaya_factory
```
> الـ `<container-host>` بيتم تحديده تلقائياً من الـ GitHub Actions workflow
> عن طريق اسم الـ PostgreSQL container الشغّال.

### الدخول لـ psql
```bash
# من جوّه الـ container
docker exec -it mazaya-postgres psql -U mazaya -d mazaya_factory
...
| **Local Tag** | `furniture-xhl2yk:latest` |
| **Port** | `3000` (محلّي) → Nginx بيعمل reverse proxy عليه |
| **Restart Policy** | Docker Swarm بيديرها تلقائياً |
| **PostgreSQL Container** | `mazaya-postgres` |
...
# شوف اللوجز
docker logs $(docker ps --filter name=furniture-xhl2yk -q | head -1) --tail 50 -f
...
### متغيرات البيئة
```
DATABASE_URL="postgres://mazaya:mazaya_pass_123@<pg-host>:5432/mazaya_factory"
JWT_SECRET=your-random-secret
NEXT_PUBLIC_SITE_URL=https://mazaya.openappo.com
NODE_ENV=production
PORT=3000
HOSTNAME=0.0.0.0
```
...
1. **مفيش backup تلقائي** لقاعدة البيانات — يفضّل تعمل cron job:
   ...
   0 3 * * * docker exec mazaya-postgres pg_dump -U mazaya mazaya_factory | gzip > /root/backups/mazaya-$(date +\%Y\%m\%d).sql.gz
   ```
